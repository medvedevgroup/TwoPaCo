...
// initialization done, search in need
...
namespace Sibelia
{
	...
	public:
		...
		void Init(std::istream & bifurcationTempRead, uint64_t verticesCount, uint64_t vertexLength, size_t threads)
		{
			...
			{
				junction_map.insert({{bifurcationKey_.at(j).ToString(vertexLength), j}});
			}
		}
		...
		uint64_t GetId(std::string::const_iterator pos) const
		{
			...
			string pos_key = bitBuf.ToString(vertexLength_);
			bitBuf.Clear();
			bitBuf.CopyFromReverseString(pos, vertexLength_);
			string neg_key = bitBuf.ToString(vertexLength_);
			
			std::unordered_map<std::string,size_t>::const_iterator pos_iter = junction_map.find(pos_key);
			if(pos_iter != junction_map.end())
			{
				posFound = true;
				ret = pos_iter->second;
			}
			...
			if(!posFound)
			{
				std::unordered_map<std::string,size_t>::const_iterator neg_iter = junction_map.find(neg_key);
				if(neg_iter != junction_map.end())
				{
					negFound = true;
					ret = neg_iter->second;
				}
	...
	private:
		...
		std::unordered_map<std::string,size_t> junction_map;
