		...
		vertexSize_(vertexLength)
	{
		...
		for (const std::string & nowFileName : fileName)
		{
			for (StreamFastaParser parser(nowFileName); parser.ReadRecord();)
			{
				char ch;
				DnaString posEdge;
				std::vector<std::unique_ptr<KarpRabinHash<uint64> > > hf(q);
				for (std::unique_ptr<KarpRabinHash<uint64> > & ptr : hf)
				{
					ptr.reset(new KarpRabinHash<uint64>(edgeLength, 64));
				}
		...
		std::cout << "RabinKarp: " << double(clock() - start) / CLOCKS_PER_SEC << std::endl;
